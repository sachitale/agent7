...
def test_python_function(chunker):
    ...
    chunks = chunker.chunk(path, "testrepo", "test.py", "python", None)
...
def test_empty_file_returns_one_chunk(chunker):
    path = _tmpfile("", ".py")
    chunks = chunker.chunk(path, "testrepo", "empty.py", "python", None)
    assert len(chunks) == 1
...
def test_fallback_for_unsupported_language(chunker):
    ...
    chunks = chunker.chunk(path, "testrepo", "file.rb", "ruby", None)
...
def test_scala_class_and_object(chunker):
    ...
    chunks = chunker.chunk(path, "testrepo", "Main.scala", "scala", None)
    names = [c.name for c in chunks]
